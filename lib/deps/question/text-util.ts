  ...
  for (const item of items) {
    ...
    for (let i = 0; i < searchTerms.length; i++) {
      if (lastSequenceMatch === -1) {
        ...
        if (itemParts[lastSequenceMatch + 1] === searchTerms[i]) {
          ...
          const start = itemAsStringSearchable.indexOf(
            searchTerms[i],
            itemParts.slice(0, lastSequenceMatch).join(" ").length,
          );
          const end = itemAsStringSearchable.indexOf(
            searchTerms[i],
            itemParts.slice(0, i).join(" ").length,
          );
      ...
      const interWordMatches = itemParts.map((part, index) => {
        ...
        if (matchIndex !== -1) {
          return matchIndex +
            itemAsStringSearchable.indexOf(
              part,
              itemParts.slice(0, index).join(" ").length,
            );
        }
        return -1;
      }).filter((res) => res > -1);
      matches.push(
        ...interWordMatches.map((index) => ({
          start: index,
          end: index + searchTerms[i].length,
        })),
      );
    ...
    for (const match of matches) {
      ...
      continuousMatches[matchIndex] = {
        start: continuousMatches[matchIndex].start < match.start
          ? continuousMatches[matchIndex].start
          : match.start,
        end: continuousMatches[matchIndex].end < match.end
          ? match.end
          : continuousMatches[matchIndex].end,
      };
    }
    ...
    specificityList.push({
      ...
      specificityScore: 0b100 * specificity.exactWordMatchesInSequence +
