...
console.log(
  "Result: %s",
  await question("password", "Password? (will print result)"),
);
console.log(
  "Result: %s",
  await question(
    "password",
    "Password with custom pattern? (will print result)",
    "<>",
  ),
);
console.log(
  ...
  await question(
    "password",
    "Password with no printing characters? (will print result)",
    false,
  ),
);
console.log(
  "Result: %s",
  await question("list", "List with object? (will print result)", {
    "Cheese": "cheese",
    "Milk": "milk",
    "Tofu": "tofu",
  }),
...
console.log(
  "Result: %s",
  await question("checkbox", "Select stuff with object? (will print result)", {
...
await question("list", "Select Service:", commonPorts, {
  windowSize: 7,
  filtering: true,
});
await question(
  ...
  "Select Services:",
  Object.fromEntries(
    Object.entries(commonPorts).map(([label, value]) => [label, { value }]),
  ),
  {
    windowSize: 7,
    filtering: true,
