...
export default async function confirm(
  ...
  let cursorIndex = 0;
  const _positiveText = typeof defaultValue === "object" &&
      typeof defaultValue.positiveText !== "undefined"
    ? defaultValue.positiveText
    : "Yes";
  const _negativeText = typeof defaultValue === "object" &&
      typeof defaultValue.negativeText !== "undefined"
    ? defaultValue.negativeText
    : "No";
  const positiveText = _positiveText.substring(0, 1).toUpperCase() +
  ...
  return createRenderer({
    ...
    prompt: () =>
      print(
        PREFIX + asPromptText(prompt) + text +
          moveCursor(Math.abs(cursorIndex - text.length), "left"),
      ),
    actions: [
      ...
      [KeyCombos.parse("enter"), async ({ clear }) => {
        ...
        await println(
          PREFIX + asPromptText(label) +
            highlightText(result ? positiveText : negativeText),
        );
        return { result };
      }],
    ],
    async defaultAction(keypress, options) {
      if (
        !keypress.ctrlKey && !keypress.metaKey && keypress.keyCode !== undefined
      ) {
        text = text.slice(0, cursorIndex) + keypress.sequence +
