import type { Plugin, MiddlewareHandler } from "../deps.ts";

export interface NetzoAuthOptions {
  visibility?: "private" | "public";
}

const createHandler = (options: NetzoAuthOptions = { visibility: "private" }): MiddlewareHandler => {
    ...
    switch (options.visibility) {
      case "private": {
        ...
        if (![origin].includes(req.headers.get("origin")!)) {
          const error = `Forbidden: private projects can only be accessed from https://${origin}`;
