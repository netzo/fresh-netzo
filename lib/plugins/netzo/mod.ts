import { UnoGenerator, type UserConfig } from "../deps.ts";
import { Plugin } from "../deps.ts";
import { presetNetzo } from "../../ui/mod.ts";
...
export default function unocss({
  ...
  const uno = new UnoGenerator(unocss); // extendUnocssOptions(config)
  return {
    name: "netzo",
    ...
    middlewares: [
      {
        path: ' _404',
        middleware: { handler: () => new Response('Not found', { status: 404 }) }
      }
    ],
  };
}
