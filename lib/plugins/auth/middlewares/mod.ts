...
const skip = (ctx: FreshContext<NetzoState>) => {
...
export function setState(config: AuthConfig): MiddlewareFn<NetzoState> {
  return async (ctx) => {
...
export function setSessionState(_config: AuthConfig): MiddlewareFn<NetzoState> {
  return async (ctx) => {
    if (skip(ctx)) return await ctx.next();
...
export function assertUserIsMemberOfWorkspaceOfApiKeyIfProviderIsNetzo(
  _config: AuthConfig,
): MiddlewareFn<NetzoState> {
  return async (ctx) => {
    if (skip(ctx)) return await ctx.next();
...
export function assertUserIsAuthorized(config: AuthConfig): MiddlewareFn<NetzoState> {
  return async (ctx) => {
    if (skip(ctx)) return await ctx.next();
...
export function setRequestState(_config: AuthConfig): MiddlewareFn<NetzoState> {
  return async (ctx) => {
    if (skip(ctx)) return await ctx.next();

    // const host = ctx.req.headers.get("host"); // e.g. my-project-906698.netzo.io
...
export function ensureSignedIn(_config: AuthConfig): MiddlewareFn<NetzoState> {
  return async (ctx) => {
    if (skip(ctx)) return await ctx.next();
