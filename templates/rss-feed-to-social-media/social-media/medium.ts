...
const mediumUserId = "146350f74b67428f12a258bde8c8a62d0a5bc272dfa363ca7e0a756486611234e"
...
export async function getPublicationContributors() {
  const response = await medium.publications[`${mediumPublicationId}`].contributors.get()
  const userPublications = await response.json
  console.log(`[medium] Successfully retrieved user publications`)
  console.log(userPublications)
  return new Response(JSON.stringify(userPublications, null, 2), {
...
export async function newPostUnderPublicationToMedium(post: any) {
  const response = await medium.publications[`${mediumPublicationId}`].posts.post({
    title: post.title,
    contentFormat: "markdown",
  ...
  console.log(`[medium] Successfully posted new entry under publication: ${post.title}`)
